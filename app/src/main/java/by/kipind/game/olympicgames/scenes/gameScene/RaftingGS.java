...
public class RaftingGS extends BaseScene implements IOnSceneTouchListener {
   ...
   private static final Object TAG_GAME_ELEMENT_FROM_FILE_SHOOT_AREA_L = "area_l";
    ...
    private void createBackground() {
        ...
        for (int i = 0; i < 10; i++) {
            deltaX = deltaX - 37f;
            deltaY = (float) (deltaY + 15+(-30*Math.round(Math.random()))); //(20f * (Math.random()+ - 10f));

            attachChild(new Sprite(deltaY, deltaX, resourcesManager.gameGraf.get("kaiak_woda"), vbom));
            berega.add(new Sprite(deltaY - 100, deltaX, resourcesManager.gameGraf.get("kaiak_pesok"), vbom));
            berega.add(new Sprite(deltaY + 100, deltaX, resourcesManager.gameGraf.get("kaiak_pesok"), vbom));
